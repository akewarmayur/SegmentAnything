...
class generateData:
    ...
    def classify(self, SOURCE_IMAGE_PATH, name, extension, OBJECTS, list_of_prompts, mask_generator, model, preprocess,
                 CLIP_THRESHOLD, df):
            ...
            for aa, m in enumerate(masks):
                ...
                if height >= 50 and width >= 50:
                    ...
                    if c1 == "a photo of others":
                        tmp = [SOURCE_IMAGE_PATH, "Not Found"]
                        df_length2 = len(df)
                        df.loc[df_length2] = tmp
                    elif c1 == "a photo ":
                        tmp = [SOURCE_IMAGE_PATH, "Not Found"]
                        df_length2 = len(df)
                        df.loc[df_length2] = tmp
    ...
    def startProcess(self, folder_name, OBJECTS, CLIP_THRESHOLD):
        ...
        df = pd.DataFrame(columns=["ImagePath", "Predictions"])
        for fi in glob.glob(folder_name + "/*"):
            try:
                ...
                df = self.classify(SOURCE_IMAGE_PATH, name, extension, OBJECTS, list_of_prompts, mask_generator, model,
                                   preprocess, CLIP_THRESHOLD, df)
            except Exception as e:
                print(e)
        df.to_csv("ImagesPredictions.csv")


if __name__ == '__main__':
    my_parser = argparse.ArgumentParser()
    my_parser.add_argument('--folder_name', action='store', type=str, required=True)
    my_parser.add_argument('--class_names', action='store', type=str, required=True)
    args = my_parser.parse_args()
    folder_name = args.folder_name
    bbox = args.bbox
    OBJECTS = args.class_names.split(",")
    CLIP_THRESHOLD = 95
    videos = False
    objT = generateData()
